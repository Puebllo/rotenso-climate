...
namespace esphome
{
  namespace rotenso
  {
    ...
    ParsedClimateState parse_heartbeat(const std::vector<uint8_t> &buffer)
    {
      ...
      {
      case 0x2:
        result.preset = esphome::climate::CLIMATE_PRESET_NONE;
        break;
      case 0x3:
        result.preset = esphome::climate::CLIMATE_PRESET_NONE;
        break;
      case 0x7:
        result.preset = esphome::climate::CLIMATE_PRESET_ECO;
        break;
      case 0xB:
        result.preset = esphome::climate::CLIMATE_PRESET_BOOST;
        break;
      case 0xF:
        result.preset = esphome::climate::CLIMATE_PRESET_BOOST;
        break; // on remote is selected eco + boost but AC works like it was on BOOST
      default:
        result.preset = esphome::climate::CLIMATE_PRESET_NONE;
        break;
      }
      ...
      if (buffer[10] == 0x00)
      { // Swing Off
        result.swing_mode = climate::CLIMATE_SWING_OFF;
      }
      else if (/*buffer[51] == 0x08 && buffer[52] == 0x00 &&*/ buffer[10] == 0x60) // co z 0x40 i 0x20????
      {                                                                            // Swing Vertical
        result.swing_mode = climate::CLIMATE_SWING_VERTICAL;
      }
      else if (buffer[51] == 0x00 && buffer[52] == 0x08)
      { // Swing Horizontal
        result.swing_mode = climate::CLIMATE_SWING_HORIZONTAL;
      }
      else if (buffer[51] == 0x08 && buffer[52] == 0x08)
      { // Swing Both
        result.swing_mode = climate::CLIMATE_SWING_BOTH;
      }
